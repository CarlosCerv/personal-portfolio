...
async function migratePosts() {
  try {
    ...
    if (!fs.existsSync(postsDirectory)) {
      console.log('WARNING: No posts directory found');
      process.exit(0);
    }
    ...
    if (files.length === 0) {
      console.log('WARNING: No markdown files found in posts directory');
      process.exit(0);
    }
    ...
    for (const filename of files) {
      ...
      if (existingPost) {
        console.log(`SKIPPED: "${data.title}" - already exists in database`);
        skippedCount++;
        continue;
      }
      ...
      console.log(`SUCCESS: Migrated "${data.title}"`);
      migratedCount++;
    }
