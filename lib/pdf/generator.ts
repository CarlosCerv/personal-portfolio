...
export async function generateDiagnosticPDF(qaData: any, userData: any): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Main Content
      doc.moveDown(6);
      doc.fontSize(42).fillColor(secondaryColor).text('QA Diagnostic', { align: 'center', characterSpacing: -1 });
      doc.moveDown(0.2);
      doc.fontSize(20).fillColor(mutedColor).text(`${userData.nombre} · ${userData.empresa || 'Project Analysis'}`, { align: 'center' });
      ...
      doc.fontSize(10).fillColor(mutedColor).text('Confidential · AI Generated · Reviewed by Carlos Cervantes', 150, (doc as any).y + 10, { align: 'center', width: 290 });

      // ================================= PAGE 2: ANÁLISIS =================================
      doc.addPage();
      
      doc.fontSize(28).fillColor(secondaryColor).text('Identified Risks', 60, 60);
      doc.moveDown(1.5);
      ...
      (qaData.riesgos || []).forEach((riesgo: any) => {
          ...
          doc.fontSize(10).fillColor(mutedColor).text(`Module: ${riesgo.modulo}`, 145, startY + 18);
      ...
      doc.moveDown(2);
      doc.fontSize(28).fillColor(secondaryColor).text('Pro Recommendations');
      doc.moveDown(1);
      
      (qaData.recomendaciones || []).forEach((rec: any, idx: number) => {
          doc.fontSize(16).fillColor(primaryColor).text(`${idx + 1}. ${rec.titulo}`);
          doc.fontSize(10).fillColor(mutedColor).text(`Impact: ${rec.impacto} | Time: ${rec.tiempo}`);
          doc.moveDown(0.2);
      ...
      doc.fontSize(28).fillColor(secondaryColor).text('Suggested Action Plan', 60, 60);
      doc.moveDown(2);
      ...
      doc.fontSize(18).fillColor(secondaryColor).text('Recommended Next Step', 80, nextStepY);
      doc.moveDown(0.5);
      doc.fontSize(14).fillColor(primaryColor).text(`Package: ${qaData.paqueteRecomendado.toUpperCase()}`, 80);
      doc.moveDown(0.5);
      doc.fontSize(11).fillColor(secondaryColor).text('We can start with this detailed analysis next week.', 80, (doc as any).y, { width: 430 });
      doc.moveDown(0.5);
      doc.fontSize(11).fillColor(primaryColor).text('Email me: carlos.cervart@icloud.com', { link: 'mailto:carlos.cervart@icloud.com' });
      ...
      for (let i = 0; i < range.count; i++) {
        ...
        doc.fontSize(8).fillColor(mutedColor).text(
          '© 2026 Carlos Cervantes · Quality and Performance Engineering Services · Confidential report for internal use.',
          60, 
          doc.page.height - 40, 
          { align: 'center' }
        );
        doc.text(`Page ${i + 1} of ${range.count}`, 60, doc.page.height - 50, { align: 'right', width: 475 });
      }
